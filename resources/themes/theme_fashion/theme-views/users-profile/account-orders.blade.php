                ...
                <div class="mb-4">
                    <ul class="nav nav-tabs nav--tabs-2" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a href="{{ route('account-oder', ['show_order' => 'ongoing']) }}"
                                class="nav-link {{ request('show_order') == 'ongoing' || request('show_order') == null ? 'active' : '' }}">{{ translate('ongoing') }}</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a href="{{ route('account-oder', ['show_order' => 'previous']) }}"
                                class="nav-link {{ request('show_order') == 'previous' ? 'active' : '' }}">{{ translate('previous') }}</a>
                        </li>
                    </ul>
                    <form action="{{ route('account-oder') }}" method="GET">
                        <div class="search-form-2 search-form-mobile">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}"
                                class="form-control text-title" placeholder="{{ translate('search_by_order_ID') }}"
                                autocomplete="off">
                            <button type="submit" class="clear border-0 text-title">
                                @if (request('search') != null)
                                    <a href="{{ route('account-oder', ['show_order' => request('show_order')]) }}"
                                        class="text-danger">{{ translate('clear') }}</a>
                                @else
                                    ...
                                    <tr class="bg-section">
                                        ...
                                        <td>
                                            <div class="cart-product ">
                                                <label class="form-check thisIsALinkElement"
                                                    data-linkpath="{{ route('account-order-details', ['id' => $order->id]) }}">
                                                    @if ($order->seller_is == 'seller')
                                                        <img loading="lazy" alt="{{ translate('shop') }}"
                                                            src="{{ getValidImage(path: 'storage/app/public/shop/' . (isset($order->seller->shop) ? $order->seller->shop->image : ''), type: 'shop') }}">
                                                ...
                                                <div class="cont">
                                                    <div class="name text-title text-capitalize cursor-pointer thisIsALinkElement"
                                                        data-linkpath="{{ route('account-order-details', ['id' => $order->id]) }}">
                                                        {{ translate('order') }}# {{ $order->id }}
                                                    </div>
                                                    @if ($order->seller_is == 'seller')
                                                        <div class="text-base mb-1">
                                                            {{ isset($order->seller->shop) ? $order->seller->shop->name : translate('shop_not_found') }}
                                                        </div>
                                                    @elseif($order->seller_is == 'admin')
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center column-gap-2">
                                                <span
                                                    class="text-base">{{ \App\Utils\Helpers::currency_converter($order['order_amount']) }}</span>
                                            </div>
                                        </td>
                                        <td>
                        ...
                        @foreach ($orders as $key => $order)
                            <div class="border-bottom d-flex align-items-center justify-content-between pb-3 gap-2">
                                <div class="cart-product ">
                                    ...
                                    <div class="cont">
                                        <div class="name text-title text-capitalize thisIsALinkElement"
                                            data-linkpath="{{ route('account-order-details', ['id' => $order->id]) }}">
                                            {{ translate('order') }}# {{ $order->id }}</div>
                                        @if ($order->seller_is == 'seller')
                                            <div class="text-base mb-1">
                                                {{ isset($order->seller->shop) ? $order->seller->shop->name : translate('shop_not_found') }}
                                            </div>
                ...
                @else
                    <div class="text-center pt-5 w-100">
                        <div class="text-center mb-5">
                            <img loading="lazy" src="{{ theme_asset('assets/img/icons/order.svg') }}"
                                alt="{{ translate('order') }}">
                            <h5 class="pt-3 pb-2 text-muted">{{ translate('no_Order_Found') }}!</h5>
                            <p class="text-center text-muted">
                                {{ translate('you_have_not_placed_any_' . request('show_order') . '_order_yet') }}</p>
                        </div>
                    </div>
                @endif
